FUNCTION mcmc_swap_vla, _EXTRA=global_params


;+
;
; Makes/adds to one MCMC chain. To be called in parallel so multiple
; chains can be made at once with tempering. If betas =
; [1,1,1,1,1,1] and N_SWAP = NITER, then this is effectively a 
...
FOR t = 0L, N_swap - 1 DO BEGIN
   ...
   ;;- PRIOR CONSTRAINTS
   ccct = 0L
   cG = 0L & cS = 0L & cSx = 0L & cN = 0L
   WHILE (CHECK_SMIN EQ 0L) DO BEGIN  ;Smin prior depends on all variables.
               
      WHILE (newG LT global_params.prior_minalpha) OR $
         (newG GT global_params.prior_maxalpha) DO BEGIN
         newG  = ( randomn(seed) * global_params.adaptA  ) + oldG
         cG += 1
         IF cG GT 500 THEN message, 'Chain '+strim(global_params.chain_num) + $
                                    ': stuck on alpha sampling.'
      ENDWHILE

      WHILE (newS LT global_params.prior_minSmin) OR $
         (newS GT global_params.prior_maxSmin) DO BEGIN
            newS  = ( randomn(seed) * global_params.adaptS  ) + oldS
            cS += 1
         IF cS GT 500 THEN message, 'Chain '+strim(global_params.chain_num) + $
                                    ': stuck on Smin sampling.'
      ENDWHILE

      WHILE (newSx LT global_params.prior_minSx) OR $
         (newSx GT global_params.prior_maxSx) DO BEGIN
            newSx = ( randomn(seed) * global_params.adaptSx ) + oldSx
         cSx += 1
         IF cSx GT 500 THEN message, 'Chain '+strim(global_params.chain_num) + $
                                    ': stuck on Smax sampling.'
      ENDWHILE

      WHILE (newN LT global_params.prior_minNorm) OR $
         (newN GT global_params.prior_maxNorm) DO BEGIN
         newN  = ( randomn(seed) * global_params.adaptN ) + oldN
         cN += 1
         IF cN GT 500 THEN message, 'Chain '+strim(global_params.chain_num) + $
                                    ': stuck on C sampling.'
      ENDWHILE
   ...
   FOR ibin = 0, global_params.NBINS - 2 DO BEGIN
      k = kvals[ibin]    
      IF k EQ 0 THEN log_P = 0. ELSE BEGIN
