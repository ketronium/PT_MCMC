...
; version is for REAL DATA SETS. If you want to run the method
; on a simulations, run mcmc_pt.pro.
;
;
; INPUTS:
;
...
IF (n_elements(PARAM_FILE) EQ 0) THEN PARAM_FILE = 'mcmc_vla.param'
...
IF (N_swap EQ 0) THEN                                       N_SWAP = 100. 
IF (NITER EQ 0) THEN                                         NITER = 1e5
IF (INTEGRAL_RESOLUTION EQ 0) THEN             INTEGRAL_RESOLUTION = 7e3 
IF (n_elements(SWAP_SENSITIVE) EQ 0) THEN           SWAP_SENSITIVE = 0.9
...
IF N_SWAP LT global_params.NBINS THEN message, 'N_SWAP must be >= NBINS.'
...
;;- THIS should be changed to read the file in mcmc.param.
;;--------- k vals from extracted flux file -------------
r = string(randomn(seed,1)*1d)
p = strpos(r,'.')
randstr = strmid(r, p+1, 6)

IF keyword_set(RANDPOS) THEN BEGIN
   readcol, !SAVEDIR + 'smap_random.txt', smap
   message, 'Doing analysis with random pixels.', /INF
   smap *= 1e3
   tname = 'chain-random-'+randstr+'.txt'
ENDIF ELSE BEGIN
   readcol, !SAVEDIR + global_params.CATALOG_NAME, smap
   message, 'Doing analysis with SCAT extracted fluxes.', /INF
;   smap *= 1e3
;   smap = smap[where(smap LT 1e3)]
   tname = 'chain-'
ENDELSE

mult = cat_to_bin_units( _EXTRA=global_params )
smap *= mult

res = bindata(fluxbins, smap)
kvals = res[*, 1] 
message,'kvals: ',/INF
print, kvals
message,'fbins: ',/INF
...
converged = 0L
ct = 0
NLOOP = floor(double(NITER)/double(n_swap))
global_count = 0
...
FOR jkl = 1, nameloop DO BEGIN
   ...
   CASE jkl OF
      1: BEGIN
         name1 = !SAVEDIR + tname + randstr+'.txt' 
         ...
         END
      2: BEGIN
         name2 = !SAVEDIR + tname + randstr+'.txt'  
         ...
         END
      3: BEGIN
         name3 = !SAVEDIR + tname + randstr+'.txt' 
         ...
         END
      4: BEGIN
         name4 = !SAVEDIR + tname + randstr+'.txt' 
         ...
         END
      5: BEGIN
         name5 = !SAVEDIR + tname + randstr+'.txt' 
         ...
         END
      6:BEGIN
         name6 = !SAVEDIR + tname + randstr+'.txt' 
...
;openw, f1, !SAVEDIR + tname, /GET_LUN
;printf, f1,'#alpha    smin      smax       C'
;printf, f1,'#-----------------------------------'
...
WHILE i LT NITER DO BEGIN
   ...
   obridge->execute,'c1 = mcmc_swap_vla( _EXTRA=gp1 )',/nowait 
   ...
   obridge2->execute,'c2 = mcmc_swap_vla( _EXTRA=gp2 )',/nowait 
   ...
   obridge3->execute,'c3 = mcmc_swap_vla( _EXTRA=gp3 )',/nowait
   ...
   obridge4->execute,'c4 = mcmc_swap_vla( _EXTRA=gp4 )',/nowait
   ...
   obridge5->execute,'c5 = mcmc_swap_vla( _EXTRA=gp5 )',/nowait
   ...
   obridge6->execute,'c6 = mcmc_swap_vla( _EXTRA=gp6 )',/nowait
